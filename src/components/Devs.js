...
const Devs = () => {
    ...
    const handleSearch = async (event) => {
      ...
      if(TERM?.length > 0 && TERM !== '') {
        const results = devs.filter(dev => dev?.stacks?.includes(TERM) || dev?.name?.includes(TERM) || dev?.techskills?.includes(TERM));
        results.length ? setDevs(results) : setSearchMsg('No results found');
        // console.log(TERM);
        // console.log(results);
      }
    };
  ...
  if(devs?.length > 0)
    return (<div className="py-5">
      ...
      <Formik>
        ...
        {searchMsg && <div className="text-center text-red-600">{searchMsg}</div>}
  {devs.map( (dev, index) => (
